...
# Check that BPF links are pinned for a PID
assert_bpf_linked() {
    local pid="$1"
    if [ -e "/sys/fs/bpf/pidnl/${pid}/egress" ] || [ -e "/sys/fs/bpf/pidnl/${pid}/ingress" ]; then
        return 0
    fi
    test_fail "no BPF links pinned under /sys/fs/bpf/pidnl/${pid}"
    return 1
}

# Check that BPF links are NOT pinned for a PID (after unset/clean)
assert_bpf_not_linked() {
    local pid="$1"
    if [ -e "/sys/fs/bpf/pidnl/${pid}/egress" ] || [ -e "/sys/fs/bpf/pidnl/${pid}/ingress" ]; then
        test_fail "BPF links still pinned after cleanup"
        return 1
    fi
    return 0
}
